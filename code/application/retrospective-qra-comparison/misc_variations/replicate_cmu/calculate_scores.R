...
#  c("national", "state", "state_national", "county"),
  ...
  function(spatial_scale) {
    ...
    submissions_root <- paste0(
      "code/application/retrospective-qra-comparison/misc_variations/replicate_cmu/retrospective-forecasts/",
      spatial_scale, "/"
    )
...
all_scores <- all_scores %>%
  mutate(
    ...
    window_size = as.integer(gsub(".*window_size_(\\d).*", "\\1", all_scores$model))
  )

saveRDS(
  all_scores,
  "code/application/retrospective-qra-comparison/misc_variations/replicate_cmu/retrospective_scores.rds")
